...
当通过复制得到新的 Empty 时，若带有旧的曲线名缓存，生成时会自动重建为该发射器独占的曲线名，避免多个发射器误写到同一条曲线。
同一场景下复制发射器后，重算会继承源曲线的预览形态参数（宽度、密度、UV 等）与预览 Mesh 材质槽，便于快速复制同风格瀑布分支。
...
`Global Simulation` 分组提供 `Generate All Emitters`，可一次性重算场景内全部已启用的 Empty 发射器，且每个发射器仍保持独立参数与独立曲线关联。
...
自由模拟在命中可支撑表面后，会沿表面切向重力继续流动，并结合接触摩擦逐步耗散速度；贴附阶段会基于邻域探针构建“宏观支撑面”，再用 `Surface Flow Radius / Samples / Relaxation / Inertia` 参数控制外层流线的平滑包裹感，避免逐三角面追随导致的锯齿拐折。持续失去法线支撑后才恢复自由落体。
碰撞仅接受正面命中（front-face），背面命中会被忽略，用于避免薄片背面把水流错误吸附回去。
宽度控制也采用“基准宽度 + 相对倍率”的方式：基准宽度由曲线整体空间跨度和 `Base Width Ratio` 决定，路径中途绕远或碰撞滑行不会把宽度额外放大，`Start/End Scale` 只负责做相对缩放。
垂直 cross strip 还支持 `Cross Ramp Length`：在瀑布起始/脱附段先保持较窄，再在过渡距离内逐步加宽，减少崖口“突然立片”的突兀感。
